...
from content_agent.handler import ContentStepHandler
...
async def run() -> None:
    ...
    worker = RabbitRpcWorker(
        ...
        queue_name=settings.content_rpc_queue,
        handler=handler,
    )

    log_event(agent_name, "worker_ready", queue=settings.content_rpc_queue)
    try:
