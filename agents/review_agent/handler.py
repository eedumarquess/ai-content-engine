...
def group_review_context_blocks(
    documents: Sequence[RerankedDocument],
) -> ReviewContextBlocks:
...
class ReviewStepHandler:
    ...
    def __init__(
        ...
        *,
        ...
        reranker: ReviewReranker,
        prompt_loader: PromptLoader,
        llm_client: ReviewLlmClient,
    ...
    async def handle(self, request: StepRpcRequest) -> WorkerExecutionResult:
        if request.step_name != "review":
            return self._validation_failure(
                request=request,
                message="Review agent only handles the review step.",
                field="step_name",
            )

        if request.input_json.document is None:
            return self._validation_failure(
                request=request,
                message="Review agent requires input_json.document.",
                field="input_json.document",
            )

        try:
            source_document = GenerationDocumentV1.model_validate(
                request.input_json.document
            )
        except ValidationError:
            return self._validation_failure(
                request=request,
                message="Review agent received an invalid input document.",
                field="input_json.document",
            )

        query = build_review_query(request, source_document)
        ...
        context_blocks = group_review_context_blocks(reranked_docs)
        prompt = self.prompt_loader.render(
            ...
            input_document_json=json.dumps(
                source_document.model_dump(mode="json", exclude_none=False),
                ensure_ascii=True,
                indent=2,
                sort_keys=True,
            ),
        ...
        try:
            ...
            finalized_document = finalize_review_document(
                ...
                request,
                performance_context_used=bool(
                    source_document.metadata.performance_context_used
                    or context_blocks.performance_context_used
                ),
            )
